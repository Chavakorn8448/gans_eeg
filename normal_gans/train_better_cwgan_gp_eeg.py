...
Standalone training script for an improved conditional WGAN-GP on BCICIV-2b EEG (T.gdf),
based on your notebook, with the following upgrades applied:

- Residual + multi-scale Conv1D generator with multi-layer condition injection
- Projection discriminator (forces label usage)
- PSD auxiliary loss (generator only) + small smoothness regularizer
- TTUR (lr_G > lr_D)
- Lower GP coefficient (lambda_gp=5 by default)
- Constant n_critic (NOT scheduled) as you requested

Outputs (in --outdir):
- checkpoints/*.pt (periodic + last)
- loss_curves.png
- samples_left.png, samples_right.png (stacked channels: real vs fake)
- metrics.json (PSD MSE + percentile MAE on a small eval split)

Example (tmux):
  python train_better_cwgan_gp_eeg.py --data-dir BCICIV_2b_gdf --epochs 500 --outdir runs/vanilla_improved
"""
...
# MNE is required for reading .gdf
import mne

# tqdm progress bars
from tqdm.auto import tqdm

# matplotlib only used for saving pngs (safe on headless)
import matplotlib
matplotlib.use("Agg")
...
def seed_everything(seed: int = 42):
    ...
    # Note: deterministic can slow training; enable if you need strict reproducibility
    torch.backends.cudnn.deterministic = False
    torch.backends.cudnn.benchmark = True


# -------------------------
# Data preprocessing (faithful to your notebook)
# -------------------------
def preprocess_one_file(
    ...
    # map "769"/"770" -> actual annotation codes in this file
    event_id = {
    ...
    # EOG rejection by peak-to-peak on EOG channels (in µV)
...
def normalize_to_unit(
    ...
    """
    Converts volts -> µV, clips to [-clip_uv, clip_uv], scales to [-1,1]
    """
    X_uv = X_volts * 1e6
    X_uv = np.clip(X_uv, -clip_uv, clip_uv)
    Xn = (X_uv / clip_uv).astype(np.float32)
    return Xn
...
def stratified_split(X: np.ndarray, y: np.ndarray, val_frac: float = 0.1, seed: int = 42):
...
# -------------------------
# Model: Generator (Residual + MultiScale + conditioning injection)
# -------------------------
...
class ResBlock1d(nn.Module):
    ...
    def forward(self, x, cond):
        ...
        h = self.act(self.conv1(x) + b)
        h = self.act(self.ms(h))
        h = self.conv2(h) + b
...
# -------------------------
# Model: Critic (Projection Discriminator)
# -------------------------
...
def batch_log_psd(x: torch.Tensor) -> torch.Tensor:
    """
    Returns mean log-PSD over batch, per channel and frequency bin: (C,F)
    """
...
# -------------------------
# Evaluation metrics (size-invariant)
# -------------------------
@torch.no_grad()
def log_psd_mse_torch(X_real: np.ndarray, X_fake: np.ndarray, device: str, max_samples: int = 256) -> float:
    """
    Compute MSE between mean log-PSD of real and fake.
    Operates in normalized space ([-1,1]) so units match.
    """
    def subsample(X):
...
def plot_loss_curves(D_hist, G_hist, out_png: Path):
    """
    Plot curves in the "classic GAN-looking" style:
    - G_hist: generator curve (e.g., -E[D(fake)])
    - D_hist: discriminator curve (we will pass -wdist so it is typically negative)
    Both share the same axis like your example (since scales are comparable now).
    """
...
def plot_stacked_pair(real_uv: np.ndarray, fake_uv: np.ndarray, tmin: float, tmax: float,
    ...
    """
    real_uv/fake_uv: (C,T) in microvolts
    Saves a single figure with two stacked panels (real on top, fake on bottom).
    """
    C, Tn = real_uv.shape
...
class TrainConfig:
    ...
    epochs: int = 500
    z_dim: int = 128
    n_critic: int = 5  # keep constant (your request)
    lr_d: float = 1e-4
    lr_g: float = 2e-4
    lambda_gp: float = 5.0
...
def main():
    ...
    ap.add_argument("--outdir", type=str, default="runs/vanilla_improved", help="Output directory")
    ...
    ap.add_argument("--lambda-smooth", type=float, default=1e-4)
    ap.add_argument("--clip-uv", type=float, default=100.0)
    ...
    # split
    X_tr, y_tr, X_val, y_val = stratified_split(X, y, val_frac=0.1, seed=cfg.seed)
    print(f"[DATA] Split: train={X_tr.shape[0]}, val={X_val.shape[0]}")
    ...
    # -------------------------
    D_hist, G_hist = [], []
    print(f"[TRAIN] device={cfg.device} epochs={cfg.epochs} batch_size={cfg.batch_size} n_critic={cfg.n_critic}")
    print(f"[TRAIN] lrD={cfg.lr_d} lrG={cfg.lr_g} lambda_gp={cfg.lambda_gp} lambda_psd={cfg.lambda_psd} lambda_smooth={cfg.lambda_smooth}")
    ...
    for epoch in epoch_pbar:
        ...
        for x_real, y_real in batch_pbar:
            ...
            for _ in range(cfg.n_critic):
                ...
                loss_D = (
                    ...
                    + cfg.lambda_gp * gradient_penalty(D, x_real, x_fake, y_real)
                )
            ...
            # Wasserstein estimate (no GP): wdist = E[D(real)] - E[D(fake)]
            with torch.no_grad():
                d_real = D(x_real, y_real).mean().item()
                d_fake = D(x_fake, y_real).mean().item()  # x_fake is last fake from critic loop
                wdist_vals.append(d_real - d_fake)
            ...
            loss_G_total = loss_G_wgan + (cfg.lambda_psd * loss_psd) + (cfg.lambda_smooth * loss_smooth)
        ...
        # Make "Discriminator Loss" look like classic negative curve:
        # D_curve = -wdist
        D_hist.append(-mean_wdist)
        ...
        if (epoch + 1) % cfg.save_every == 0 or (epoch + 1) == cfg.epochs:
            ...
            save_checkpoint(cfg.outdir / "checkpoints" / ckpt_name, epoch + 1, G, D, opt_G, opt_D, cfg)
            save_checkpoint(cfg.outdir / "checkpoints" / "last.pt", epoch + 1, G, D, opt_G, opt_D, cfg)

            # update loss plot for tmux convenience
            plot_loss_curves(D_hist, G_hist, cfg.outdir / "loss_curves.png")

    # -------------------------
    # Final eval (val split) + save visuals
    # -------------------------
    G.eval()

    rng = np.random.default_rng(cfg.seed)
    val0 = np.where(y_val == 0)[0]
    val1 = np.where(y_val == 1)[0]
    n0 = min(len(val0), cfg.eval_num_per_class)
    n1 = min(len(val1), cfg.eval_num_per_class)
    idx0 = rng.choice(val0, n0, replace=False) if n0 > 0 else np.array([], dtype=int)
    idx1 = rng.choice(val1, n1, replace=False) if n1 > 0 else np.array([], dtype=int)

    Xv0 = X_val[idx0] if n0 > 0 else None
    Xv1 = X_val[idx1] if n1 > 0 else None
    ...
    if n0 > 0:
        metrics["left"] = {
            "log_psd_mse": log_psd_mse_torch(Xv0, Xf0, device=cfg.device),
            "global_percentile_mae": global_percentile_mae(Xv0, Xf0),
            "n_real": int(n0),
            "n_fake": int(n0),
        }
    if n1 > 0:
        metrics["right"] = {
            "log_psd_mse": log_psd_mse_torch(Xv1, Xf1, device=cfg.device),
            "global_percentile_mae": global_percentile_mae(Xv1, Xf1),
            "n_real": int(n1),
            "n_fake": int(n1),
        }
    ...
    # sample plots (convert back to µV for readability)
    if n0 > 0:
        real_uv = Xv0[0] * cfg.clip_uv
        fake_uv = Xf0[0] * cfg.clip_uv
        plot_stacked_pair(
            real_uv, fake_uv, cfg.tmin, cfg.tmax,
            "LEFT: Real vs Fake (stacked channels)",
            cfg.outdir / "samples_left.png",
        )

    if n1 > 0:
        real_uv = Xv1[0] * cfg.clip_uv
        fake_uv = Xf1[0] * cfg.clip_uv
        plot_stacked_pair(
            real_uv, fake_uv, cfg.tmin, cfg.tmax,
            "RIGHT: Real vs Fake (stacked channels)",
